...
var routes = [
  ...
  {
    path: "login",
    loadComponent: () => import("./chunk-GMWHM7XU.js").then((m) => m.LoginComponent)
  },
  {
    path: "dashboard",
    loadComponent: () => import("./chunk-LSQOYAV6.js").then((m) => m.DashboardComponent),
    canActivate: [authGuard]
  },
  {
    path: "lists/new",
    loadComponent: () => import("./chunk-HFG6GAZ7.js").then((m) => m.ListCreateComponent),
    canActivate: [authGuard]
  },
  {
    path: "lists/:id/edit",
  ...
  {
    path: "lists/:id",
    loadComponent: () => import("./chunk-5NQNYVQL.js").then((m) => m.ListDetailComponent),
    canActivate: [authGuard]
  },
  {
    path: "**",
...
var ItemStatus2;
(function(ItemStatus3) {
  ItemStatus3["ToBuy"] = "to_buy";
  ItemStatus3["Bought"] = "bought";
...
var _DexieDataBase = class _DexieDataBase extends import_wrapper_default {
  ...
  createItemInList(listId, item) {
    return from(this.transaction("rw", this.lists, this.items, this.products, () => __async(this, null, function* () {
      let list = yield this.lists.get(listId);
      if (!list || !list.id) {
        throw new Error(`Could not find list with id ${listId}`);
      }
      const product = yield this.products.where("name").equalsIgnoreCase(item.product.name).first();
      let productId;
      if (product && product.id) {
        productId = product.id;
      } else {
        productId = yield this.products.add(item.product);
      }
      return this.items.add(__spreadProps(__spreadValues({}, item), { productId, listId: list.id }));
    })));
  }
  updateItem(id, item) {
    return from(this.items.update(id, item));
  }
  deleteItem(id) {
    return from(this.items.delete(id));
  }
  createList(listData) {
  ...
  getAllLists() {
    return from(liveQuery(() => this.transaction("r", this.lists, this.items, this.products, () => __async(this, null, function* () {
      ...
      for (const itemDB of itemDBs) {
        let productDB = productMap.get(itemDB.productId);
        if (!productDB) {
      ...
      return lists.map((listDB) => {
        let items = itemsByList.get(listDB.id) ?? [];
  ...
  getListById(id) {
    return from(liveQuery(() => this.transaction("r", this.lists, this.items, this.products, () => __async(this, null, function* () {
      const listDB = yield this.lists.get(id);
      if (!listDB) {
        throw new Error(`List ${id} not found`);
      }
      const itemsDB = yield this.items.where("listId").equals(id).toArray();
      ...
      const itemsWithProduct = itemsDB.map((itemDB) => {
        let productDB = products.find((p) => p?.id === itemDB.productId);
  ...
  updateList(id, list) {
    return from(this.lists.update(id, list));
  }
  deleteList(id) {
    return from(this.lists.delete(id));
  }
};
